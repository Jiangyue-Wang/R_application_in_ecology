...
## load the data
d.clim <- read.csv(file="/Users/lilywang/Downloads/ClimateChina.csv", header=T, stringsAsFactors=F)
names(d.clim)

d.clim.t1 <- d.clim.t2 <- d.clim.t3 <- d.clim[,c(4:14)]

## not good
###PCA analysis in princomp function
t.pca1 <- princomp(d.clim.t1, cor = F)

summary(t.pca1)
t.pca1$loadings[]#original data multiply loadings
...
## good
###original climate data cor=TRUE, use correlation matrix
t.pca3 <- princomp(d.clim.t3, cor = T)
summary(t.pca3)
t.pca3$loadings[]
r.mat <- cor(d.clim.t3, t.pca3$scores)
r.mat
biplot(t.pca3, xlabs = rep("o", times=dim(d.clim.t1)[1]), col=c("gray", "red"))
abline(h=0, v=0, lty=2)


## another way to plot the data
##to generate the plot by hands
windows()
...
##多因素相互影响，温度、降水、植物、动物
## Energy, indicating by temperature, can influence mammals directly
## or indirectly by influencing plants; precipitation is the same
## SEM based on multi-variable linear models
###########################################################
install.packages("sem") 
require(sem)

d.XJ <- read.csv("/Users/lilywang/Downloads/speciesRichness3ResolutionXinjiang.csv",header=T)
str(d.XJ); dim(d.XJ)
#PET indicates energy; AET indicates actual water resource
#map linear regression to decide use PET or PET squared or others
PET <- d.XJ$PET
PET2 <- PET^2
AET <- d.XJ$AET
...
## load the data
d.woody.pro <- read.csv("/Users/lilywang/Downloads/woody_prov.csv", header=T)
